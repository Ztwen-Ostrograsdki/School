		...
		<div class="w-100">
			...
			<div class="pupils">
				...
				<div class="d-flex w-100 my-1 justify-content-start">
					<div class="mx-1">
						<span class="fa fa-refresh text-white-50" @click="filtrer('all')" title="Cliquez pour lister tous les apprenants" v-if="alert != 'Tous les apprenants'"></span>
						<button class="btn btn-primary mx-1" @click="filtrer('primary')" title="Cliquez pour lister les apprenants du primaire">Le Primaire () </button>
						<button class="btn btn-primary" @click="filtrer('secondary')" title="Cliquez pour lister les apprenants du secondaire">Le Secondaire ()</button>
					</div>
					<h5 class="card-link mt-3 text-white-50 ml-3"></h5>
				</div>
					...
					<table class="table-table table-striped w-100">
						...
						<tbody>
							...
							<tr v-for="(pupil, k) in pupils" :key="pupil.id" class="border-bottom border-dark">
								...
								<td class="text-left">
									<router-link to='/pupilsm/profil' class="card-link d-inline-block">
										{{pupil.name}}
									</router-link>
									<a href="#" title="card-link Editer les informations de" class="fa fa-edit text-white-50 float-right" style="font-size: 10px!important; font-weight: 200!important"></a> 
								</td>
								<td>
									{{gender(pupil.sexe)}}
								</td>
								...
								<td>
									<span class="d-inline-block w-100">
										<button title="Voulez vous supprimer" class="btn btn-danger w-100" @click='destroy(pupil.id)'>Blocké</button>
									</span>
								</td>
							</tr>
						</tbody>
    ...
    export default {
    	...
    	data() {
    		return {
    			...
    			teachersAll: [],
    			teachersLength: 0,
    			secondaryTeachersLength: 0,
    			primaryTeachersLength: 0,
    			alert: "Tous les apprenants",
    			success : false,
    			months : [
		            "Janvier",
		            "Février",
    	...
    	created(){
    		axios.get('/admin/director/teachersm/DATA&for&teachers')
    			 	.then(response => {
    			 			this.teachers = response.data.t
    			 			this.teachersAll = response.data.t
    			 			this.secondaryTeachers = response.data.tSec
    			 			this.primaryTeachers = response.data.tPrim
    			 			this.allDATA = response.data.all
    			 			this.teachersLength = this.teachers.length
			    			this.secondaryTeachersLength = this.secondaryTeachers.length
			    			this.primaryTeachersLength = this.primaryTeachers.length
    			 		}
    			 	)
    	},
    	...
    	methods: {
    		filtrer(tag){
    			...
    			if(tag == 'secondary'){
    				this.teachers = this.secondaryTeachers
    				this.alert = "Les enseignants du secondaire"
    			}
    			else if (tag == 'primary') {
    				this.teachers = this.primaryTeachers
    				this.alert = "Les enseignants du primaire"
    			}
    			else if (tag == 'all') {
    				this.teachers = this.teachersAll
    				this.alert = "Tous les enseignants"	
    			}	
    		...
    		destroy(user){
    			axios.delete('/admin/director/teachersm/' + user)
    				.then(response => {
    			 			this.teachers = response.data.t
    			 			this.teachersAll = response.data.t
    			 			this.secondaryTeachers = response.data.tSec
